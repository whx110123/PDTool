...
{
	typ = *(buff.data() + mLen);
	mText.append(CharToHexStr(buff.data() + mLen) + "\t" + typToText() + "\r\n");
	mLen++;

	sDateTime = charToDateTime(buff.data() + mLen, 7, BINARYTIME2A);
	mText.append(timeToText(buff.data() + mLen, 7));
	mLen += 7;
	eDateTime = charToDateTime(buff.data() + mLen, 7, BINARYTIME2A);
	mText.append(timeToText(buff.data() + mLen, 7));
	mLen += 7;
