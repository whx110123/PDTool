...
bool IEC103Asdu32Data::handle(const QByteArray& buff)
{
	int meaNum = 0;
	switch(inf)
	{
	...
	{
		hr = charTouint(buff.data() + len, 3);
		mText.append(hrToText(buff.data() + len, 3) + "\r\n");
		len += 3;
	}
...
bool IEC103Asdu32Data::createData(IECDataConfig& config)
{
