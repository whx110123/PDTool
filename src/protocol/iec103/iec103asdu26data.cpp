...
IEC103Asdu26Data::IEC103Asdu26Data(const MyConfig& Config): IEC103AsduData(Config)
{
	datauchar = 0;
	tov = 0;
	fan = 0;
...
bool IEC103Asdu26Data::handle(const QByteArray& buff)
{
	...
	mText.append(CharToHexStr(buff.data() + mLen, 2) + "\t电网故障序号NOF:" + QString::number(nof) + "\r\n");
	mLen += 2;

	noc = *(buff.data() + mLen);
	mText.append(CharToHexStr(buff.data() + mLen) + "\t通道数目NOC:" + QString::number(noc) + "\r\n");
	mLen++;

	noe = charTouint(buff.data() + mLen, 2);
	mText.append(CharToHexStr(buff.data() + mLen, 2) + "\t一个通道信息元素的数目NOE:" + QString::number(noe) + "\r\n");
	mLen += 2;

	_int = charTouint(buff.data() + mLen, 2);
	mText.append(CharToHexStr(buff.data() + mLen, 2) + "\t信息元素间的间隔INT:" + QString::number(_int) + "   单位:微秒\r\n");
	mLen += 2;
