...
int main(int argc, char *argv[])
{
	...
	App::Protocollst << IEC_104 << IEC_101 << IEC_61850
	...
	App::ConfigFile = QString("%1/%2.ini").arg(QUIHelper::appPath(), QUIHelper::appName());
	App::readConfig();
	App::readSendData();
	App::readDeviceData();

	frmMain w;
	w.setWindowTitle("规约调试工具    " + APPTITLE);
	w.show();

	w.resize(800, 600);

	//居中显示窗体
	int deskWidth = QGuiApplication::primaryScreen()->availableGeometry().width();
	int deskHeight = QGuiApplication::primaryScreen()->availableGeometry().height();
